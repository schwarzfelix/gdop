"""Distance error plot showing average distance error in each scenario, grouped by arrangement.

This file contains the code to draw a grouped bar chart comparing the average distance error
...
class ArrangementDistanceErrorPlot(QObject):
    ...
    def update_data(self, anchors=False, tags=False, measurements=False):
        """Compute average distance error for each scenario and update the grouped bar chart.
        ...
        for s in self.scenarios:
            ...
            # Get average distance error from measurements
            try:
                errors = s.get_measurement_errors()
                if errors:
                    # Calculate average absolute error
                    avg_error = sum(abs(err) for err in errors.values()) / len(errors)
                else:
                    avg_error = 0.0
            except Exception:
                avg_error = 0.0
            
            arrangement_data[arrangement][variant].append(float(avg_error))
        ...
        # Plot bars - using DISTANCE_ERROR color for both to show they are distance errors
