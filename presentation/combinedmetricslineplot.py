"""Combined metrics line plot showing position error and tag truth GDOP as lines over scenarios.

This file contains code to draw a line plot comparing position error and tag truth GDOP
across scenarios, allowing for trend comparison.
"""
...
class CombinedMetricsLinePlot(QObject):
    ...
    Expected usage:
      plot = CombinedMetricsLinePlot(window, scenarios)
      plot.update_data()
      plot.redraw()
    """
    ...
    def __init__(self, window, app_scenarios):
        ...
        self.ax.set_title('Scenario Metrics Trends: Position Error and Tag Truth GDOP')
        self.ax.set_ylabel('Value')
        self.ax.set_xlabel('Scenario')

    def update_data(self, anchors=False, tags=False, measurements=False):
        """Compute metrics for the first tag of each scenario and update the line plot.
